...
import {
  ...
  FileSpreadsheet, CheckCircle2, AlertTriangle, Trash2
} from 'lucide-react'
              ...
              {loading ? (
                ...
                <tr key={c.id} className="hover:bg-slate-50">
                  ...
                  <td className="text-right">
                    ...
                    {canDelete && (
                      <button onClick={() => setDeleteTarget(c)} className="btn-ghost btn-sm !p-1.5 text-red-500 hover:text-red-600" title="Delete">
                        <Trash2 size={13} />
                      </button>
                    )}
                  </td>
      ...
      <Modal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Delete Client">
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            Are you sure you want to delete <span className="font-semibold text-gray-900">{deleteTarget?.companyName}</span>
            {' '}({deleteTarget?.clientName})? This cannot be undone. If the client has linked
            services, invoices, or other records, deletion may be blocked until those are removed first.
          </p>
