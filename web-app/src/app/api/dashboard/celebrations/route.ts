...
import { nowIST } from '@/lib/attendanceDate'
...
export async function GET(req: NextRequest) {
  ...
  // "now" resolved as today's IST calendar date, held as a UTC-midnight
  // instant — use getUTC* getters below so the server's own timezone
  // (which may not be IST) never matters.
  const now = nowIST()
