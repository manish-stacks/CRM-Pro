...
export async function POST(req: NextRequest) {
  // Leads can be imported by any staff member (Telecaller and up) — it was
  // previously locked to MANAGER+ which is why a telecaller's import
  // always failed with a silent 403. Client import stays MANAGER+ only,
  // checked below inside that branch specifically.
  ...
  if (Array.isArray(body.clients)) {
    if (!hasMinRole((session as any).role, 'MANAGER')) {
      return errorResponse('Forbidden', 403)
    }
  ...
  try {
    ...
    for (let i = 0; i < leads.length; i++) {
      ...
      const clientPhone = String(row.clientPhone || row.phone || '').trim()
      ...
      if (statusRaw && !VALID_STATUSES.includes(statusRaw)) {
        warnings.push({ row: i + 1, error: `Status "${row.status || row.Status}" not recognized — set to NEW` })
      }
      const status = statusRaw && VALID_STATUSES.includes(statusRaw) ? statusRaw : 'NEW'
      let createdLead
        ...
        try {
          createdLead = await prisma.lead.create({
            data: {
              ...
              clientName,
              clientPhone,
              clientEmail: row.clientEmail || row.email ? String(row.clientEmail || row.email) : null,
              companyName: row.companyName || row.company ? String(row.companyName || row.company) : null,
              address: row.address || row.Address || null,
              ...
              createdById: (session as any).userId,
              assignedToId: (session as any).userId,
            },
          })
          break
        } catch (createErr: any) {
      ...
      if (createdLead) {
        ...
        console.error(`[leads import] row ${i + 1} failed:`, lastErr)
        const reason = lastErr?.code === 'P2002'
          ? `Duplicate value (${lastErr?.meta?.target || 'unique field'})`
          : lastErr?.message
            ? String(lastErr.message).split('\n').pop()?.slice(0, 200)
            : 'Unknown error'
        errors.push({ row: i + 1, error: `Failed to create lead — ${reason}` })
      }
    }

    return successResponse({ imported: created.length, errors, warnings }, created.length)
  } catch (err) {
    console.error('[leads import] failed:', err)
