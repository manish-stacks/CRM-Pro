...
export default function PortalShell({ children }: { children: React.ReactNode }) {
  ...
  const {
    ...
    client, logout, initials, invoices, services, openTickets, pollTickets,
  ...
  if (checking) return <div className="min-h-screen flex items-center justify-center bg-slate-50"><Loader2 className="animate-spin text-brand-400" /></div>

  if (!loggedIn) {
    return (
      <div className="min-h-screen grid lg:grid-cols-2">
        {/* Brand panel */}
        <div className="hidden lg:flex flex-col justify-between p-12 bg-gradient-to-br from-brand-600 via-brand-600 to-brand-800 text-white relative overflow-hidden">
          ...
          <div className="relative space-y-4">
            ...
            <p className="text-brand-100 max-w-sm">Track services, view reports, pay invoices, and reach your account manager — anytime.</p>
            <div className="flex gap-6 pt-4">
              {[['Services', Package], ['Reports', FileText], ['Payments', TrendingUp]].map(([l, I]: any) => (
                <div key={l} className="flex items-center gap-2 text-sm text-brand-100"><I size={16} /> {l}</div>
              ))}
            </div>
          </div>
          ...
          <div className="w-full max-w-sm">
            ...
            <form onSubmit={login} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" required placeholder="you@company.com"
                    className="w-full pl-9 pr-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100"
              ...
              <button type="submit" disabled={logging}
                className="w-full bg-brand-600 hover:bg-brand-700 text-white py-2.5 rounded-xl font-medium flex items-center justify-center gap-2 transition-colors shadow-sm shadow-brand-200">
      ...
      <div className="bg-white/90 backdrop-blur border-b border-gray-200 sticky top-0 z-20">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-brand-500 to-brand-600 flex items-center justify-center text-white font-bold text-sm overflow-hidden">
          ...
          {NAV.map((t) => {
            ...
            return (
              <Link key={t.href} href={t.href}
                className={`px-3.5 py-2.5 text-sm font-medium border-b-2 flex items-center gap-1.5 whitespace-nowrap transition-colors ${active ? 'border-brand-600 text-brand-600' : 'border-transparent text-gray-500 hover:text-gray-800'
                  }`}>
                <t.icon size={14} /> {t.label}
                {badge > 0 && <span className="ml-0.5 bg-brand-100 text-brand-700 text-[10px] font-bold rounded-full px-1.5 py-0.5 leading-none">{badge}</span>}
              </Link>
            )
          })}
        </div>
      ...
      {ticketModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setTicketModal(false)}>
          <div className="bg-white rounded-2xl max-w-md w-full p-5 space-y-3" onClick={e => e.stopPropagation()}>
            ...
            <input className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-brand-500" placeholder="Subject *" value={ticketForm.subject} onChange={e => setTicketForm(p => ({ ...p, subject: e.target.value }))} />
            <textarea className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-brand-500" placeholder="Describe your issue..." rows={4} value={ticketForm.description} onChange={e => setTicketForm(p => ({ ...p, description: e.target.value }))} />
            <div className="grid grid-cols-2 gap-3">
              <select className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-brand-500" value={ticketForm.priority} onChange={e => setTicketForm(p => ({ ...p, priority: e.target.value }))}>
                <option value="LOW">Low</option><option value="MEDIUM">Medium</option><option value="HIGH">High</option><option value="URGENT">Urgent</option>
              </select>
              <select className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-brand-500" value={ticketForm.clientServiceId} onChange={e => setTicketForm(p => ({ ...p, clientServiceId: e.target.value }))}>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button disabled={ticketSaving} onClick={async () => {
                ...
                try {
                  ...
                  setTicketForm({ subject: '', description: '', priority: 'MEDIUM', clientServiceId: '' })
                  loadData()
                } finally { setTicketSaving(false) }
              }} className="px-4 py-2 rounded-xl text-sm bg-brand-600 hover:bg-brand-700 text-white font-medium">
      ...
      {payModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setPayModal(null)}>
          <div className="bg-white rounded-2xl max-w-md w-full p-5 space-y-3" onClick={e => e.stopPropagation()}>
            ...
            <div className="bg-brand-50 rounded-xl p-3 text-sm flex items-center justify-between">
              <span className="text-gray-600">{payModal.invoiceNumber}</span>
              <span className="font-bold text-brand-700">Due {fmt(payModal.dueAmount)}</span>
            </div>
            <div>
              <label className="text-xs text-gray-500 mb-1 block">Amount</label>
              <input type="number" className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-brand-500" value={payForm.amount} onChange={e => setPayForm(p => ({ ...p, amount: e.target.value }))} />
            </div>
            <div>
              <label className="text-xs text-gray-500 mb-1 block">Method</label>
              <select className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-brand-500" value={payForm.method} onChange={e => setPayForm(p => ({ ...p, method: e.target.value }))}>
            ...
            <div>
              ...
              <input className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-brand-500" placeholder="UPI ref / Cheque no / Bank ref" value={payForm.reference} onChange={e => setPayForm(p => ({ ...p, reference: e.target.value }))} />
            </div>
            <textarea className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-brand-500" placeholder="Notes (optional)" rows={2} value={payForm.notes} onChange={e => setPayForm(p => ({ ...p, notes: e.target.value }))} />
