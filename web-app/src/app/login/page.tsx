...
export default function LoginPage() {
  ...
  // Remember the last email used to log in (not the password) so the
  // field is pre-filled next time instead of being typed from scratch —
  // this is what makes the desktop app "remember my login id" too, since
  // it just loads this same page inside its own persistent session.
  const SAVED_EMAIL_KEY = 'hbs_last_login_email'
  useEffect(() => {
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.requiresOtp) {
        ...
        try { window.localStorage.setItem(SAVED_EMAIL_KEY, email.trim().toLowerCase()) } catch { }
