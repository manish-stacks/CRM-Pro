...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (action === 'punch_in') {
      ...
      const u = await prisma.user.findUnique({ where: { id: session.userId }, select: { emailVerified: true } })
      if (!u?.emailVerified) {
        return errorResponse('Please verify your email before checking in', 403)
      }
